...
import { exportCandidatesToCSV } from "../../../api/candidates.api";
export default function ExportCandidates({ isOpen, onClose }) {
  ...
  const handleExportCandidateCSV = async (candidates) => {
    console.log({ candidates });

    if (!candidates?.length) {
      setError("No verified candidates to export");
      return;
    }
    ...
    candidates.forEach((candidate, index) => {
      const employeeName = `${candidate?.data?.firstname || ""} ${
        candidate?.data?.middlename || ""
      } ${candidate?.data?.surname || ""}`.trim();

      worksheet.addRow({
        sn: index + 1,
        employeeName: employeeName || "N/A",
        employeeCode: candidate?.details?.EmployeeCode || "N/A",
        companyCode: candidate?.details?.company_code || "N/A",
        address1: candidate?.details?.current_address_1 || "N/A",
        addres2: candidate?.details?.current_address_2 || "N/A",
        town: candidate?.details?.current_address_town || "N/A",
        dateVerified: candidate?.dateVerified
          ? new Date(candidate.dateVerified).toISOString().split("T")[0]
          : "N/A",
    ...
    saveAs(new Blob([buffer]), "candidates.xlsx");
  };
